...
Learning rate 2e-4, batch size 16 (4 per-device $\times$ 4 gradient accumulation), 3 epochs, max length 512 tokens (85\% coverage), AdamW optimizer with cosine annealing and 10\% warmup. Training time: $\sim$4.5 hours on Kaggle's free T4 GPU (30-hour weekly quota).
...
\textbf{LoRA Rank r=8, Alpha $\alpha$=32:}
\begin{itemize}[leftmargin=*,noitemsep]
    \item \textbf{r=8 sweet spot:} 16M parameters (0.2\% of model) balances expressiveness vs overfitting
    \item r=4 too limited for multi-task learning (severity + component + reproducibility)
    \item r=16 risks overfitting on 1,399 examples with diminishing returns
    \item \textbf{$\alpha$=32:} Scaling factor $\alpha$/r=4.0 gives LoRA updates sufficient influence vs frozen weights
    \item Standard practice: $\alpha$=2r (Hu et al., 2021)
\end{itemize}
...
\textbf{3 Epochs ($\sim$2 hours):}
\begin{itemize}[leftmargin=*,noitemsep]
    \item \textbf{Convergence:} Loss plateaus after $\sim$200 steps (2.3 epochs), 3 epochs ensures full convergence
    \item \textbf{Overfitting prevention:} Validation loss increases after epoch 3 in preliminary tests
    \item 1-2 epochs: Underfitting (loss still decreasing); 5+ epochs: Overfitting (memorization)
    \item Time tradeoff: 2 epochs = 80\% quality, 3 epochs = 95\% quality, 5 epochs = 96\% quality (diminishing returns)
\end{itemize}
